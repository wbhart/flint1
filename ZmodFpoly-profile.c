...
void sample_ssfft_ifft(unsigned long length, unsigned long n,
                       unsigned long count)
{
   unsigned long m = ceil_log2(length);
   ...
   for (unsigned long i = 0; i < count; i++)
      ssfft_ifft(poly->coeffs, 1, m, length, length, 0, 0,
                 (4*n*FLINT_BITS_PER_LIMB) >> m, n, poly->scratch);
...
char* prof2dDriverString_ssfft_ifft(char* params)
{
   return "legacy ssfft_ifft code";
}
...
Parameters for this target are:
   ...
   n_count: number of coefficient lengths to test
*/
void prof2dDriver_ssfft_ifft(char* params)
{
   int length_min, length_max, n_count;
   double length_ratio;
   ...
   prof2d_set_sampler(sample_ssfft_ifft);
