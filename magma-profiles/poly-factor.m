...
/*
Profiling MAGMA polynomial factorisation in Z[x].

Usage: run magma with the -b flag to prevent the start up banner, i.e.

   magma -b poly-factor.txt > output.prof
...
target_description := "MAGMA polynomial factorisation in Z[x] over various lengths and bitsizes, NON-NEGATIVE coefficients only";


max := 10000;   // maximum total bitsize of input polys
   ...
   for i := 0 to max_iter do
      ...
      if length ne last_length then
         ...
         for j := 0 to max_iter do
            bits := Floor(ratio^j);
            if bits ne last_bits then
               last_bits := bits;

               if length*bits le max then
