...
install_headers = ["zn_poly/zn_poly.h", "zn_poly/wide_arith.h"]

# These are the other headers.
other_headers = ["zn_poly/support.h", "zn_poly/profiler.h", "zn_poly/zn_poly_internal.h"]
...
for x in lib_modules + prof_modules + testprof_modules + tune_modules + example_progs:
   print
   print "%s.o: zn_poly/%s.c $(HEADERS)" % (x, x)
   print "\t$(CC) $(CFLAGS) -DNDEBUG -o %s.o -c zn_poly/%s.c" % (x, x)
