...
ZNP_INLINE
cycle_count_t get_cycle_counter()
{
...
ZNP_INLINE
cycle_diff_t cycle_diff(cycle_count_t start, cycle_count_t stop)
{
   return stop - start;
}
...
/*
   Repeatedly runs _target_ with parameter _arg_, with increasing values
   of _count_ (count not allowed to get bigger than 10).
   
   Expects _target_ to be a function that runs a certain profile _count_ times
   ...
   Runs for at most _limit_ seconds (as measured by the target!).
   Calls _target_ at least once, and at most 50 times.
   
   If spread != NULL, it stores there the ratio
       (interquartile range) / (median).
       
   If samples != NULL, it stores there the number of times _target_ was called.
*/
