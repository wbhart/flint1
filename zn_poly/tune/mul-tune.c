...
/*
   ...
   thresholds have been determined, since they are used as subroutines.)
...
tune_mul (FILE* flog, int sqr, int verbose)
{
   ...
   fprintf (flog, "    KS/FFT %s: ", sqr ? "sqr" : "mul");
   fflush (flog);
   ...
   for (b = 2; b <= ULONG_BITS; b++)
   {
      ...
      if (sqr)
         tuning_info[b].sqr_fft_thresh = thresh;
      else
         tuning_info[b].mul_fft_thresh = thresh;
   }
