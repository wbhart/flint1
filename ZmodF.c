                            ...
                            int squaring)
{
   ...
   // for now just use mpn_mul_n
   info->use_fft = 0;

   // todo: this should use stack-based memory manager; and the documentation
   // for this function NEEDS TO MENTION THIS
                       ...
                       ZmodF_t res, ZmodF_t a, ZmodF_t b)
{
   ...
   {
      ZmodF_mul(res, a, b, info->scratch, info->n);
   }
}
...
void ZmodF_sqr_precomp(ZmodF_mul_precomp_t info, ZmodF_t res, ZmodF_t a)
{
   ...
   {
      ZmodF_sqr(res, a, info->scratch, info->n);
   }
}
