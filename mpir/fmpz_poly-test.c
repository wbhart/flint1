...
#if DEBUG
          for (ulong i = 0; i < test_poly3->length; i++)
          {
             if (mpz_cmp(test_poly3->coeffs[i], test_poly4->coeffs[i]) != 0)
             {
                gmp_printf("coeffs1[%ld] = %Zd, coeffs2[%ld] = %Zd\n", i, test_poly3->coeffs[i], i, test_poly4->coeffs[i]);
             }
          }
#endif
      }
