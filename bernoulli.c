...
int bernoulli_mod_p(unsigned long *res, unsigned long p)
{
   ...
   // print_var("g", g);
   // print_var("g_inv", g_inv);
   // print_var("g_sqr", g_sqr);
   // print_var("g_sqr_inv", g_sqr_inv);
   
   unsigned long poly_size = (p-1)/2;
   // print_var("poly_size", poly_size);
   ...
   {
      // printf("%i ------------\n", i);
      // print_var("g_power", g_power);
      // print_var("g_power_inv", g_power_inv);
      // 
      // print_var("fudge", fudge);
      // print_var("fudge_inv", fudge_inv);
      
      // compute h(g^i)/g^i (h(x) is as in latex notes)
      temp = g * g_power;
      
      // print_var("temp", temp);
      //       print_var("expr", p + constant - (temp / p));
      ...
      temp2 = z_mulmod_precomp(h, fudge, p, p_inv);
      // print_var("temp2", temp2);
      mpz_poly_set_coeff_ui(G, i, temp2);
   ...
   // printf("F(X) = %s\n", mpz_poly_to_string(F));
   // printf("G(X) = %s\n", mpz_poly_to_string(G));
   
   mpz_poly_t product;
   mpz_poly_init(product);
   mpz_poly_mul(product, G, F);
   
   // printf("F(X) * G(X) = %s\n", mpz_poly_to_string(product));
