...
\end{lstlisting}
\begin{quote}
Returns a random uniformly distributed integer in the range $\[0, \mbox{limit})$. If \code{limit} is set to 0, the function returns a random limb, i.e. a random integer in the range $[0, 2^\mbox{FLINT_BITS})$.  
\end{quote}
...
\end{lstlisting}
\begin{quote}
Returns a random uniformly distributed integer with up to the given number of bits. If \code{bits} is set to 0, the function returns a random limb, i.e. a random integer in the range $[0, 2^\mbox{FLINT_BITS})$.  
\end{quote}
