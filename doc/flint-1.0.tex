...
\subsection{String conversions and I/O}
The functions in this section read/write a polynomial to/from a string representation. The representation starts with the length of the polynomial, a space and then the modulus of the polynomial. If the length is not zero, this is followed by two spaces ANDY and then a space separated list of the coefficients starting from the constant coefficient. Each coefficient is represented as an integer between zero and one less than the modulus.
...
\end{lstlisting}
\begin{quote}
Return the maximum number of bits used in the coefficients of \code{poly}, i.e. if $n$ is returned, then no coefficient of the polynomial uses more than $n$ bits.  ANDY\end{quote}
...
\end{lstlisting}
\begin{quote}
Returns 1 if the polynomial is equal to the constant polynomial 0, otherwise returns 0.  ANDY
\end{quote}
...
\end{lstlisting}
\begin{quote}
Set \code{res} to \code{poly} squared. The length of \code{res} will be at most one less than twice the lengths of \code{poly}.  ANDY
\end{quote}
...
\end{lstlisting}
\begin{quote}
The first function is for caching of FFT for a truncated multiplicaton of \code{input2}, with the given number of bits per output coefficient (0 if this is to be computed automatically) where the output will be truncated to the given length.  The second function performs the truncated multiplication.  ANDY
\end{quote}
...
\end{lstlisting}
\begin{quote}
If (1.5)*\code{trunc} is larger than the degree of \code{input1} plus the degree of \code{input2}-1, then the top (\code{trunc}-$\lfloor$\code{trunc}/2$\rfloor$) coefficients will be the middle product of \code{input1} and the pre-cached FFT for \code{input2}, \code{pre}.  This is because the product of \code{input1} and \code{input2} will wrap around when writing to \code{output} which will have length \code{trunc}.  For example, when performing a Newton inversion the degree of \code{input1} is $\lfloor$(\code{trunc}+1)/2$\rfloor$, the degree of \code{input2} is \code{trunc} and the degree of their product is  \code{trunc}+$\lfloor$(\code{trunc}+1)/2$\rfloor$-1.  So $\lfloor$(\code{trunc}+1)/2$\rfloor$-1 terms wrap around leaving the top \code{trunc}-$\lfloor$ \code{trunc}/2 $\rfloor$ terms accurate and as the resulting middle product.
...
\end{lstlisting}
\begin{quote}
The first function is for caching of FFT for a multiplicaton of \code{input2}, with the given number of bits per output coefficient (0 if this is to be computed automatically) where the output will be truncated to the given length.  The second function performs the multiplication.  ANDY
\end{quote}
...
\end{lstlisting}
\begin{quote}
Function for clearing a \code{zmod_poly_mul_precache_t}.  ANDY
\end{quote}
...
\end{lstlisting}
\begin{quote}
Sets \code{res} equal to the derivative of \code{poly} and reduces all the coefficients $\pmod{p}$.
\end{quote}
...
\end{lstlisting}
\begin{quote}
Sets \code{res} equal to the product of \code{poly1} and \code{poly2} modulo \code{f}.  Assumes that \code{poly1} and \code{poly2} are reduced modulo \code{f}.  ANDY
\end{quote}
...
\end{lstlisting}
\begin{quote}
Returns \code{pol} raised to the power \code{exp} modulo \code{f}.  Assumes \code{pol} is reduced modulo \code{f}.  There are no restrictions on \code{exp}, i.e. it can be zero or negative.  The leading coefficient of \code{f} must be invertible modulo the modulus.  ANDY
\end{quote}
...
\end{lstlisting}
\begin{quote}
The first function initializes an array of factors.  The second function clears an array of factors.
ANDY
\end{quote}
...
\end{lstlisting}
\begin{quote}
Prints to stdout each factor in the array \code{fac}.
ANDY
\end{quote}
...
\end{lstlisting}
\begin{quote}
Sets \code{red} to a square-free factorization of \code{f}.
ANDY
\end{quote}
...
\end{lstlisting}
\begin{quote}
Sets \code{result} to be a complete factorization of \code{input}.  No restriction on \code{input}.
ANDY
\end{quote}
...
\end{lstlisting}
\begin{quote}
Calculates the Jacobi symbol of (\code{x}/\code{y}). Assumes that gcd(\code{x},\code{y})=1 and \code{y} is odd.  ANDY
\end{quote}
...
\end{lstlisting}
\begin{quote}
Checks to see if \code{n} is a Fermat pseudoprime with base \code{i}.  Assumes that \code{n} does not divide \code{i}.  ANDY
\end{quote}
...
\end{lstlisting}
\begin{quote}
This is currently only a deterministic primality test if FLINT can factor \code{n} - 1.  Note that this is currently the case if \code{n} - 1 has less than \code{FLINT_BITS - 10} bits after removal of factors below 1000.  In the second function it is assumed that \code{n} is greater than 2 and odd.  ANDY
\end{quote}
...
\end{lstlisting}
\begin{quote}
This is a deterministic prime test up to $10^16$. Requires \code{n} to be at most \code{FLINT_BITS-1} bits.  For numbers greater than $10^16$ there are no known counterexamples to the conjecture that a composite will never be declared prime.  ANDY
\end{quote}
...
\end{lstlisting}
\begin{quote}
A second version of the Lucas pseudoprime test.  Assumes \code{n} has been checked for primality using trial factoring up to 256.  ANDY
\end{quote}
...
\end{lstlisting}
\begin{quote}
Returns the integer part of the square root of \code{r}.  ANDY
\end{quote}
...
\end{lstlisting}
\begin{quote}
Tests if \code{n} squarefree or not.  Currently only works for numbers up to 65535.  ANDY
\end{quote}
...
\end{lstlisting}
\begin{quote}
Removes the highest power of \code{p} possible from \code{n} and returns the exponent to which it appeared in \code{n}.  In the second function \code{n} can only be up to \code{FLINT_BITS-1} bits.  ANDY
\end{quote}
...
\end{lstlisting}
\begin{quote}
Finds all of the factors of \code{n} by trial factoring up to some limit.  Returns the cofactor after removing these factors.  ANDY
\end{quote}
...
\end{lstlisting}
\begin{quote}
Square forms factoring algorithm of Shanks.  Adapted from the (simplified) algorithm as described by Gower and Wagstaff Math of Comp. (Preprint May 2007) ANDY
\end{quote}
...
\end{lstlisting}
\begin{quote}
Find the factors of \code{n}.  This function may fail if \code{n} is so large that SQUFOF multipliers (rarely up to 1000) times \code{n} push it over a limb in size.  If factoring fails (very rare), this function returns 0, else it returns 1.  ANDY
\end{quote}
...
\end{lstlisting}
\begin{quote}
Factors \code{n} until the product of the factor found is $>$ \code{limit}. It puts the factors in \code{factors} and returns the cofactor.  If factorisation cannot be achieved as \code{n} is too large (for SQUFOF), 0 is returned.  ANDY
\end{quote}
...
\end{lstlisting}
\begin{quote}
Returns a random prime integer with the given number of bits.  Assumes \code{bits}>1.  ANDY
\end{quote}
