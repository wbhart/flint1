...
void sample_Zpoly_mpn_mul_karatsuba(unsigned long length, unsigned long bits,
                          unsigned long count)
{
   unsigned long m = ceil_log2(length);
   unsigned long output_bits = 2*bits+m;
   ...
   _Zpoly_mpn_stack_init(poly1, length, (bits-1)/FLINT_BITS_PER_LIMB+1);
   _Zpoly_mpn_stack_init(poly2, length, (bits-1)/FLINT_BITS_PER_LIMB+1);
   _Zpoly_mpn_stack_init(poly3, 2*length-1, poly1->limbs+poly2->limbs+2);
   ...
   for (unsigned long i = 0; i < count; i++)
   {
      if (i%r_count == 0)
      {
       ...
       prof2d_start();
       _Zpoly_mpn_mul_karatsuba(poly3, poly1, poly2);
       prof2d_stop();
   }
...
char* prof2dDriverString_Zpoly_mpn_mul_karatsuba(char* params)
{
   return "Zpoly_mpn_mul_karatsuba over various lengths and various bit sizes";
}
...
Parameters for this target are:
   ...
   n_count: number of coefficient lengths to test
*/
void prof2dDriver_Zpoly_mpn_mul_karatsuba(char* params)
{
   int length_min, length_max, bits_min, bits_max;
   double ratio;
   ...
   prof2d_set_sampler(sample_Zpoly_mpn_mul_karatsuba);
...
void sample_Zpoly_mpn_mul(unsigned long length, unsigned long bits,
                          unsigned long count)
{
   unsigned long m = ceil_log2(length);
   unsigned long output_bits = 2*bits+m;
   ...
   for (unsigned long i = 0; i < count; i++)
   {
      if (i%r_count == 0)
      {
       ...
       prof2d_start();
       _Zpoly_mpn_mul(poly3, poly1, poly2);
       prof2d_stop();
   }
...
char* prof2dDriverString_Zpoly_mpn_mul(char* params)
{
   return "Zpoly_mpn_mul over various lengths and various bit sizes";
}
...
Parameters for this target are:
   ...
   n_count: number of coefficient lengths to test
*/
void prof2dDriver_Zpoly_mpn_mul(char* params)
{
   int length_min, length_max, bits_min, bits_max;
   double ratio;
   ...
   prof2d_set_sampler(sample_Zpoly_mpn_mul);
