...
int test_F_mpz_getset_mpfr()
{
   ...
   ulong count1;
   for (count1 = 0; (count1 < 1000000*ITER) && (result == 1); count1++)
   {
      bits = z_randint(200)+ 1;      
...
int test_F_mpz_smod_preinv()
{
   ...
   for (count1 = 0; (count1 < 100000*ITER) && (result == 1); count1++)
   {
      ...
      mp_ptr finv = F_mpz_precompute_inverse(f2);
		...
		F_mpz_smod_preinv(f3, f1, f2, finv);
		if (F_mpz_sgn(f3) < 0)
         F_mpz_add(f3, f3, f2);
      F_mpz_get_mpz(m3, f3);
      mpz_mod(m4, m1, m2);
	...
	for (count1 = 0; (count1 < 100000*ITER) && (result == 1); count1++)
   {
      ...
      F_mpz_get_mpz(m2, f2);
		...
		F_mpz_smod_preinv(f1, f1, f2, finv);
		if (F_mpz_sgn(f1) < 0)
         F_mpz_add(f1, f1, f2);
      F_mpz_get_mpz(m3, f1);
      mpz_mod(m4, m1, m2);
	...
	for (count1 = 0; (count1 < 100000*ITER) && (result == 1); count1++)
   {
      ...
      F_mpz_get_mpz(m2, f2);
		...
		F_mpz_set(f3, f2);
      F_mpz_smod_preinv(f2, f1, f2, finv);
		if (F_mpz_sgn(f2) < 0)
         F_mpz_add(f2, f2, f3);
      F_mpz_get_mpz(m3, f2);
      mpz_mod(m4, m1, m2);
...
int test_F_mpz_gcd()
{
   ...
   for (count1 = 0; (count1 < 100000*ITER) && (result == 1); count1++)
   {
      ...
      result = (mpz_cmp(m4, m3) == 0); 
		if (!result) 
		{
			gmp_printf("Error: bits = %ld, bits2 = %ld, bits3 = %ld, m1 = %Zd, m2 = %Zd, m3 = %Zd, m4 = %Zd\n", bits, bits2, bits3, m1, m2, m2, m3, m4);
		}
...
int test_F_mpz_multi_CRT_ui_unsigned()
{
   ...
   ulong i;
   for (i = 0; (i < 10000*ITER) && (result == 1); i++)
   {
...
int test_F_mpz_multi_CRT_ui()
{
   ...
   for (i = 0; (i < 10000*ITER) && (result == 1); i++)
   {
     ...
     ulong k;
     for (k = 0; (k < num_primes) && (result == 1); k++)
     {
        output2[k] = F_mpz_mod_ui(temp, input, primes[k]);
		  result &= (output[k] == output2[k]);
